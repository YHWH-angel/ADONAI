...
class BlockchainTest(BitcoinTestFramework):
    ...
    def _test_verificationprogress(self):
        self.log.info(
            "Check that verificationprogress is less than 1 when the block tip is old"
        )
        future = 2 * 60 * 60
        self.nodes[0].setmocktime(
            self.nodes[0].getblockchaininfo()["time"] + future + 1
        )
        assert_greater_than(
            1, self.nodes[0].getblockchaininfo()["verificationprogress"]
        )

        self.log.info(
            "Check that verificationprogress is exactly 1 for a recent block tip"
        )
        ...
        self.log.info(
            "Check that verificationprogress is less than 1 as soon as a new header comes in"
        )
    ...
    def _test_gettxoutsetinfo(self):
        ...
        assert_raises_rpc_error(
            -8, "'foo hash' is not a valid hash_type", node.gettxoutsetinfo, "foo hash"
        )
    ...
    def _test_gettxout(self):
        ...
        assert_equal(
            txout["scriptPubKey"]["hex"], self.wallet.get_output_script().hex()
        )
    ...
    def _test_getnetworkhashps(self):
        ...
        assert_raises_rpc_error(
            ...
            lambda: self.nodes[0].getnetworkhashps(
                100, self.nodes[0].getblockcount() + 1
            ),
        )
        assert_raises_rpc_error(
            -8,
        ...
        blocks_since_last_diff_change = (
            current_block_height % DIFFICULTY_ADJUSTMENT_INTERVAL + 1
        )
        expected_hashes_per_second_since_diff_change = self.nodes[0].getnetworkhashps(
            blocks_since_last_diff_change
        )

        assert_equal(
            self.nodes[0].getnetworkhashps(-1),
            expected_hashes_per_second_since_diff_change,
        )

        # Ensure long lookups get truncated to chain length
        hashes_per_second = self.nodes[0].getnetworkhashps(
            self.nodes[0].getblockcount() + 1000
        )
    ...
    def _test_waitforblock(self):
        ...
        assert_equal(
            node.waitforblock(blockhash=current_hash, timeout=1)["hash"],
            rollback_header["previousblockhash"],
        )
        assert_raises_rpc_error(
            -1, "Negative timeout", node.waitforblock, current_hash, -1
        )
        ...
        # but poll anyway.
        self.wait_until(
            lambda: node.waitforblock(blockhash=current_hash, timeout=100)["hash"]
            == current_hash
        )
        ...
        # but poll anyway.
        self.wait_until(
            lambda: node.waitfornewblock(timeout=100)["hash"] == current_hash
        )
    ...
    def _test_waitforblockheight(self):
        ...
        assert_waitforheight(current_height + 1)
        assert_raises_rpc_error(
            -1, "Negative timeout", node.waitforblockheight, current_height, -1
        )
    ...
    def _test_getblock(self):
        ...
        block = create_block(
            int(blockhash, 16),
            create_coinbase(current_height + 1, nValue=100),
            block_time,
        )
        block.solve()
        ...
        assert_raises_rpc_error(
            ...
            lambda: node.getblock(block.hash),
        )
        ...
        tx = create_tx_with_script(
            block.vtx[0], 0, script_sig=bytes([OP_TRUE]), amount=18 * COIN
        )
        block_noundo = create_block(
            block.sha256,
            create_coinbase(current_height + 2, nValue=100),
            block_time + 1,
            txlist=[tx],
        )
        block_noundo.solve()

...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    // Set effective feerate up to MAX_MONEY angs per 1'000'000 vB (2'100'000'000 ang/vB = 21'000 BTC/kvB).
...
void FuzzCoinSelectionAlgorithm(std::span<const uint8_t> buffer) {
    ...
    const CFeeRate discard_fee_rate{fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(CFeeRate{g_fee_model.min_fee}.GetFeePerK(), COIN)};
    const CAmount target{fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(1, MAX_MONEY)};

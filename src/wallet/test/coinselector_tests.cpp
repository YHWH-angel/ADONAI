...
BOOST_AUTO_TEST_CASE(bnb_sffo_restriction)
{
    ...
    // Knapsack will only find a changeless solution on an exact match to the angel, SRD doesn’t look for changeless
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    // test randomness
    {
        ...
        for (int i = 0; i < RUN_TESTS; i++) {
            // picking 18 from 100 coins doesn't depend on the shuffle,
            // but does depend on randomness in the stochastic approximation code
            const auto result25 = KnapsackSolver(GroupCoins(available_coins.All()), 18 * COIN, CENT);
            BOOST_CHECK(result25);
            const auto result26 = KnapsackSolver(GroupCoins(available_coins.All()), 18 * COIN, CENT);
    ...
    int max_weight = MAX_STANDARD_TX_WEIGHT - WITNESS_SCALE_FACTOR * (cs_params.tx_noinputs_size + cs_params.change_output_size);
    {
        ...
        const auto result = select_coins(
            target, cs_params, cc, [&](CWallet& wallet) {
                ...
                add_coin(available_coins, wallet, CAmount(18 * COIN), CFeeRate(0), 144, false, 0, true);
                return available_coins;
            },
            m_node);

        BOOST_CHECK(result);
        // Verify that the 18 ADO UTXO was selected, and result is below max_weight
        BOOST_CHECK(has_coin(result->GetInputSet(), CAmount(18 * COIN)));

...
const char * const ADONAI_CONF_FILENAME = "adonai.conf";
const char * const ADONAI_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; // adonai-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ADONAI_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Adonai
    //   new: C:\Users\Username\AppData\Local\Adonai
    // macOS: ~/Library/Application Support/Adonai
    // Unix-like: ~/.adonai
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Adonai";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Adonai";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Adonai";
#else
    // Unix-like
    return pathRet / ".adonai";
#endif
#endif
}

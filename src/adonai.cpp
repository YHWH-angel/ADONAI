...
Additional less commonly used commands:
  ...
  chainstate [ARGS] Run adonai kernel chainstate util, equivalent to running 'adonai-chainstate [ARGS]'.
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back("adonai-chainstate");
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/adonai/adonai/pull/31375#discussion_r1861814807)
